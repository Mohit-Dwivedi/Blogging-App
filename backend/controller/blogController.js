...
export const blogPost = catchAsyncError(async (req,res,next) => {
    ...
    // (paraOneImage && !allowedFormast.includes(paraOneImage.mimetype)) ||
    ...
    if(!mainImageRes || mainImageRes.error || (paraOneImage && (!paraOneImageRes || paraOneImageRes.error)) || (paraTwoImage && (!paraTwoImageRes || paraTwoImageRes.error)) || (paraThreeImage && (!paraThreeImageRes || paraThreeImageRes.error))){
        return next(new ErrorHandler("Error Occured while uploading one more images", 500))
    }

    const blogData = {title, intro, paraOneDescription, paraOneTitle, paraTwoDescription, paraTwoTitle, paraThreeDescription, paraThreeTitle, category, createdBy, authoName, authorAvatar, mainImage:{public_id: mainImageRes.public_id, url: mainImageRes.secure_url}, published}
...
export const singleBlogs = catchAsyncError(async (req,res,next) => {
    const {id} = req.params 
    const blog = await Blog.findById(id)
    if(!blog){
        return next(new ErrorHandler("Blog not Found!", 400))
...
export const updateBlogs = catchAsyncError(async (req, res, next) => {
    const { id } = req.params;
